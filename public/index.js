...
// const creatWeekHeader = () => {
//     weeks.forEach(day => {
//         const dayElement = document.createElement('div') //div作成
//         dayElement.classList.add('week-day')
//         dayElement.textContent = day
...
//         ele = document.getElementById("p" + i) // 調べ方はプロパティの有無 eleBox[i]でクリックしたオブジェクト取得
//         ele.addEventListener("click", () => {  // pBox[i]にプロパティ名を入れたからこれを使い削除編集を行う
//             console.log(eleBox[i])             // 削除するときのメッセージはconfirm(本当に削除しますか？)でおけ
//             previewDialog.showModal()          // 編集は初期状態を現在の予定に合わせて予定入力ダイヤログを開く
//         })                                     // 開いた時に現在入ってるshortSchedule,longScheduleは削除して、新しく入れる
//     }                                          // 元の情報は仮のオブジェクトに保存してキャンセルされたらそいつらを入れる
// }
...
//             previewElement.classList.add('previewElement');
//             previewElement.textContent = previewSchedule.title
//             preview.appendChild(previewElement)
//             count++
//             previewElement.id = 'p' + count
...
//                 dialog.close(); // 現在のダイアログを閉じる
//                 setDialogStateWithSchedule(previewSchedule); // クリックした要素のスケジュール情報を設定
//                 scheduleDialog.showModal(); // スケジュール入力ダイアログを開く
//                 preview.textContent = null; // プレビューの内容をクリア
//             });
...
//     const setDialogStateWithSchedule = (schedule) => {
//         // タイトルと詳細の設定
//         title.value = schedule.title || null;  // スケジュール名をセット
//         detail.value = schedule.detail || null; // 詳細をセット

//         // 日付の設定
//         startDateTime.value = `${schedule.year}-${String(schedule.month).padStart(2, '0')}-${String(schedule.day).padStart(2, '0')}T${String(schedule.shour).padStart(2, '0')}:${String(schedule.sminutes).padStart(2, '0')}`; // 開始日付と時間
//         endDateTime.value = `${schedule.year}-${String(schedule.month).padStart(2, '0')}-${String(schedule.day).padStart(2, '0')}T${String(schedule.ehour).padStart(2, '0')}:${String(schedule.eminutes).padStart(2, '0')}`; // 終了日付と時間

//         startTime.value = `${String(schedule.shour).padStart(2, '0')}:${String(schedule.sminutes).padStart(2, '0')}`; // 開始時間の設定
//         endTime.value = `${String(schedule.ehour).padStart(2, '0')}:${String(schedule.eminutes).padStart(2, '0')}`; // 終了時間の設定

//         // スイッチの見た目を設定（適宜更新してください）
//         // ここでは終日スイッチや日またぎスイッチの状態を管理する処理を追加できます
//         if (schedule.allDayState === 1) {
//             allDay.classList.add("active");
//             setDay.classList.remove("none");
//             setTime.classList.add("none");
//         } else {
...
// // ダイアログボックスを表示、編集する関数
// // const showDialogBox = (id, schedule) => {
...
//     })
//     allDay.addEventListener("click", () => { //　終日on off切り替え
//         allDayState = Math.abs(allDayState - 1)　//絶対値
//         checker(allDay, allDayS) // ダイヤログ表示切り替え
//     })
//     overDay.addEventListener("click", () => { // 日またぎon off切り替え
//         overDayState = Math.abs(overDayState - 1)
...
//                 return
//             }
//             yearMonthDateSplit() // 年月日を文字列から配列に入れ込む関数
//             obj.title = title.value // 仮の情報の入れ物に保存
//             obj.year = yearMonthDate[0]
//             obj.month = yearMonthDate[1]
//             obj.date = yearMonthDate[2]
//             if (detail.value) {
//                 obj.detail = detail.value
//             }
//         } else if (allDayState === 0 && overDayState === 1) {
...
// }
// const yearMonthDateSplit = () => { // 年月日を文字列から数字で配列に格納
//     yearMonthDate = yearMonthDate.split(/[-]/) //文字を分割する
//     for (let i = 0; i < 3; i++) {
//         yearMonthDate[i] = parseInt(yearMonthDate[i])
//     }
// }
// const intoObjects = (o) => {
//     // 仮の入れ物の情報を一致する年月日のオブジェクトに格納
//     // 日をまたぐ場合はlongSchedule当日のみはshortScheduleをプロパティ名として保存
//     // 予定が重なる場合は入れた順番で1,2,3…と番号を振る
//     // 予定が取り除かれた場合あいている若い番号から入れる
//     if ('sYear' in o) { // longかshortかの判断　in oでオブジェクトがあるかないか
//         let objBox = []
//         for (let i = o.sYear; i <= o.eYear; i++) {
//             for (let j = o.sMonth; j <= o.eMonth; j++) {
//                 for (let k = o.sDate; k <= o.eDate; k++) {
//                     objBox.push(dateObjects[i][j][k]) // 該当する日のオブジェクトを配列に保存
//                 }
//             }
//         }
//         for (let l = 1; l < 100; l++) {
...
//     return true
// }
// const pushHTMLElement = () => { //入れ込み方模索中
//     for (let a = 0; a < 42; a++) {
//         document.getElementById("schedule" + a).textContent = null
//     }
// }
...
// const dateObjects = createDateArray();
// // console.log(dateObjects);
// // 曜日を生成して表示
...
//     let squares = []; // 正方形を格納するための配列を作成
//     for (let i = 0; i < 42; i++) {
//         let square = squareTemplate.cloneNode(true); //
//         square.removeAttribute("id"); // id を削除
//         // square.classList.add('square-' + i); // クラスを追加
//         stage.appendChild(square); // 親要素 stage に追加
//         square.id = "square" + i //idを与える
//         // square.id = "go-to-page2"
//         squares.push(square); // squares 配列に square を追加
...
//     for (let i = startDay; i < startDay + endDayCount; i++) {
//         document.getElementById('square' + i).textContent = j
//         box[i] = j;
//         j++
//         if (i === 6 || i === 13 || i === 20 || i === 27 || i === 34 || i === 41) {
//             document.getElementById('square' + i).style.color = "blue"
//         }
//         if (i === 0 || i === 7 || i === 14 || i === 21 || i === 28 || i === 35 || i === 42) {
//             document.getElementById('square' + i).style.color = "red"
//         }
//         if (j === today && thisMonth === month && thisYear === year) {
//             document.getElementById('square' + (i + 1)).style.backgroundColor = "gray"
//         }
//     }

// }

// // const clickSquareEvent = (index) => {
// //     console.log(dateObjects[year][month][box[index]]);
// //     const clickedDateObjectInfo = dateObjects[year][month][box[index]];  // クリックされた要素の情報
// //     localStorage.setItem('selectedDate', JSON.stringify(clickedDateObjectInfo));
// // }

// // const nullCalendar = () => {
// //     for (let i = 0; i < 42; i++) {
// //         document.getElementById('square' + i).textContent = null
// //         document.getElementById('square' + i).style.backgroundColor = "white"
// //     }
// // }

// const creatPrevMonth = () => {
//     nullCalendar()
//     if (month == 1) {
//         month = 12
//         year--
...
//     createSquares()
//     createCalendar()
//     prevMonth.addEventListener("click", creatPrevMonth)
//     nextMonth.addEventListener("click", createNextMonth)
// }